...
def load_proxies(proxy_file):
    try:
        ...
        return []  # Return an empty list if proxies cannot be loaded
...
async def call_api(url, data, proxy, token):
    ...
    try:
        ...
        # If no proxy is provided, use the default network (main network)
        if proxy:
...
async def main():
    ...
    if not tokens:
        print("No tokens found in token.txt. Exiting the program.")
        exit()
    ...
    for token in tokens:
        ...
        if not active_proxies:  # If no proxies are available, use the main API
            logger.info("No proxies found, using main API")
            tasks[asyncio.create_task(render_profile_info(None, token))] = None
        else:
            for proxy in active_proxies:
                tasks[asyncio.create_task(render_profile_info(proxy, token))] = proxy

    # Ensure there are tasks before calling asyncio.wait
    if tasks:
